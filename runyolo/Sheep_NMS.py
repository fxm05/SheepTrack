...
def soft_nms(boxes, scores, iou_thresh=0.5, sigma=0.5, method='gaussian', score_thresh=0.001):
    """
    ...
    """
    # 第1-2行：边界条件检查，如果输入为空直接返回空张量
    if boxes.numel() == 0:
        return torch.tensor([], dtype=torch.long, device=boxes.device)

    # 第4行：按分数降序排列，获得索引顺序
    # 例如：scores = [0.9, 0.7, 0.8] → order = [0, 2, 1]
    ...
    while order.numel() > 0:
        ...
        # 第9-10行：如果只剩最后一个框，直接结束
        if order.numel() == 1:
            break

        # 第12-14行：准备计算IoU
        # 将当前框扩展为(1,4)形状，便于广播计算
        current_box = boxes[i].unsqueeze(0)  # (1, 4)
        # 获取剩余的所有框
        remaining_boxes = boxes[order[1:]]  # (K, 4)

        # 第16-31行：计算IoU（交并比）
        # 计算相交区域的左、右、上、下边界
        inter_left = torch.max(current_box[:, 0], remaining_boxes[:, 0])
        inter_right = torch.min(current_box[:, 2], remaining_boxes[:, 2])
        inter_top = torch.max(current_box[:, 1], remaining_boxes[:, 1])
        inter_bottom = torch.min(current_box[:, 3], remaining_boxes[:, 3])

        # 计算相交区域的宽高（确保不为负）
        inter_width = (inter_right - inter_left).clamp(min=0)
        inter_height = (inter_bottom - inter_top).clamp(min=0)
        inter_area = inter_width * inter_height  # 相交面积
        ...
        # 计算IoU：相交面积 / 相并面积
        iou = inter_area / (area_current + area_remaining - inter_area + 1e-9)

        # 第33-42行：应用Soft-NMS衰减策略
        if method == 'gaussian':
            # 高斯衰减: score = score * exp(-iou^2 / sigma)
            # 当iou=0时，衰减因子=1；当iou增大时，衰减因子减小
            decay_factor = torch.exp(-(iou ** 2) / sigma)
        elif method == 'linear':
            # 线性衰减: score = score * (1 - iou) if iou > threshold
            # 只有当iou超过阈值时才进行衰减
            decay_factor = torch.where(iou < iou_thresh, torch.ones_like(iou), 1.0 - iou)
        else:
            raise ValueError(f"Unknown method: {method}")

        # 第44行：更新剩余框的分数
        # 对与当前框重叠的框进行分数衰减
        scores[order[1:]] *= decay_factor

        # 第46-49行：过滤分数低于阈值的框
        remain_mask = scores[order[1:]] > score_thresh
        remain_indices = torch.where(remain_mask)[0]

        if remain_indices.numel() == 0:
            break  # 如果没有框剩余，结束循环

        # 第51-52行：更新待处理框的列表
        order = order[1:][remain_indices]  # 移除当前框，只保留分数足够的框

        # 第54-55行：重新按分数排序（因为分数被衰减了）
        if order.numel() > 1:
            order = order[torch.argsort(scores[order], descending=True)]
...
def non_max_suppression(
        ...
        end2end=False,
        # 新增 Soft-NMS 参数
        use_soft_nms=True,  # 是否使用 Soft-NMS
        soft_nms_sigma=0.3,  # 高斯衰减参数
        soft_nms_method='gaussian',  # 方法选择: 'gaussian' 或 'linear'
        soft_nms_score_thresh=0.1  # Soft-NMS 分数阈值
):
    """
    ...
    time_limit = 10.0 + max_time_img * bs  # seconds to quit after
